...
<html>
    ...
    <div class="main main-raised">
            ...
            $recordID = explode(',',ltrim($_POST['records'], ','));
    		    ...
    		    /*				Save to MySQL        */
    		    /*-----------------------------------------------*/
            $update_query = "UPDATE sleepEnvironment SET ";
            $columns = Array('score' => 'score = CASE ', 'comment' => 'comment = CASE ');
            foreach ($recordID as $index => $id) {
              $columns['score'] .= "WHEN recordId='" .$id. "' THEN '" . $score[$index] . "' ";
              $columns['comment'] .= "WHEN recordId='" . $id . "' THEN '" . $comment[$index] . "' ";
            }
            foreach($columns as $column_name => $query_part){
              $columns[$column_name] .= " ELSE '$column_name' END ";
            }
            $where = " WHERE recordId='" . implode("' OR recordId='", $recordID) . "'";
		    ...
		    <div class="row top">
                ...
                <a class="btn btn-large btn-block"  name="Continue" href="sleep-environment-non-student-review<?php echo "?".$query; ?>">Continue</a>
      			</div>
		    </div>
		</div>
	    </div>
